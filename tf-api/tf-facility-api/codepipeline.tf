###############################################################################
# codepipeline.tf  –  CodeStar, CodeBuild, CodePipeline for facility
###############################################################################

resource "aws_codestarconnections_connection" "main" {
  name          = "magi-app-stg-facility-github"
  provider_type = "GitHub"
  tags          = local.common_tags
}

resource "aws_codebuild_project" "main" {
  name          = "magi-app-stg-facility-build"
  build_timeout = 20
  ...
  logs_config {
    cloudwatch_logs {
      group_name  = "/codebuild/magi-app-stg-facility"
      stream_name = "build"
    }
  }
...
resource "aws_codepipeline" "main" {
  name     = "magi-app-stg-facility-pipeline"
