###############################################################################
# ecs.tf  –  ECS task definition, service, autoscaling for facility
###############################################################################

resource "aws_cloudwatch_log_group" "main" {
  name              = "/ecs/magi-app-stg-facility"
  retention_in_days = 14
  tags              = local.common_tags
}

resource "aws_ecs_task_definition" "main" {
  family                   = "magi-app-stg-facility"
  network_mode             = "awsvpc"
  ...
  container_definitions = jsonencode([{
    ...
    logConfiguration = {
      logDriver = "awslogs"
      options = {
        "awslogs-group"         = "/ecs/magi-app-stg-facility"
...
resource "aws_ecs_service" "main" {
  name            = "magi-app-stg-facility-service"
...
#To able th LB
 ...
 #   container_name   = "magi-app-stg-facility"
 #   container_port   = 3000
 # }
...
resource "aws_appautoscaling_policy" "main" {
  name               = "magi-app-stg-facility-cpu-scaling"
