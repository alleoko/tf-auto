###############################################################################
# codepipeline.tf  –  CodeStar, CodeBuild, CodePipeline for guarantor
###############################################################################

resource "aws_codestarconnections_connection" "main" {
  name          = "magi-app-stg-guarantor-github"
  provider_type = "GitHub"
  tags          = local.common_tags
}

resource "aws_codebuild_project" "main" {
  name          = "magi-app-stg-guarantor-build"
  build_timeout = 20
  ...
  logs_config {
    cloudwatch_logs {
      group_name  = "/codebuild/magi-app-stg-guarantor"
      stream_name = "build"
    }
  }
...
resource "aws_codepipeline" "main" {
  name     = "magi-app-stg-guarantor-pipeline"
