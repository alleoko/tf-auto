###############################################################################
# codepipeline.tf  –  CodeStar, CodeBuild, CodePipeline for inventory
###############################################################################

resource "aws_codestarconnections_connection" "main" {
  name          = "magi-app-stg-inventory-github"
  provider_type = "GitHub"
  tags          = local.common_tags
}

resource "aws_codebuild_project" "main" {
  name          = "magi-app-stg-inventory-build"
  build_timeout = 20
  ...
  logs_config {
    cloudwatch_logs {
      group_name  = "/codebuild/magi-app-stg-inventory"
      stream_name = "build"
    }
  }
...
resource "aws_codepipeline" "main" {
  name     = "magi-app-stg-inventory-pipeline"
