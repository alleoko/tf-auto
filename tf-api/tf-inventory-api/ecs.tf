###############################################################################
# ecs.tf  –  ECS task definition, service, autoscaling for inventory
###############################################################################

resource "aws_cloudwatch_log_group" "main" {
  name              = "/ecs/magi-app-stg-inventory"
  retention_in_days = 14
  tags              = local.common_tags
}

resource "aws_ecs_task_definition" "main" {
  family                   = "magi-app-stg-inventory"
  network_mode             = "awsvpc"
  ...
  container_definitions = jsonencode([{
    ...
    logConfiguration = {
      logDriver = "awslogs"
      options = {
        "awslogs-group"         = "/ecs/magi-app-stg-inventory"
...
resource "aws_ecs_service" "main" {
  name            = "magi-app-stg-inventory-service"
...
#    container_name   = "magi-app-stg-inventory"
#    container_port   = 3000
#  }
...
resource "aws_appautoscaling_policy" "main" {
  name               = "magi-app-stg-inventory-cpu-scaling"
