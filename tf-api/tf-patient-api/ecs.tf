###############################################################################
# ecs.tf  –  ECS task definition, service, autoscaling for patient
###############################################################################

resource "aws_cloudwatch_log_group" "main" {
  name              = "/ecs/magi-app-stg-patient"
  retention_in_days = 14
  tags              = local.common_tags
}

resource "aws_ecs_task_definition" "main" {
  family                   = "magi-app-stg-patient"
  network_mode             = "awsvpc"
  ...
  container_definitions = jsonencode([{
    ...
    logConfiguration = {
      logDriver = "awslogs"
      options = {
        "awslogs-group"         = "/ecs/magi-app-stg-patient"
...
resource "aws_ecs_service" "main" {
  name            = "magi-app-stg-patient-service"
...
resource "aws_appautoscaling_policy" "main" {
  name               = "magi-app-stg-patient-cpu-scaling"
