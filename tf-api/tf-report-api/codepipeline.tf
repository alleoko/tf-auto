###############################################################################
# codepipeline.tf  –  CodeStar, CodeBuild, CodePipeline for reports
###############################################################################

resource "aws_codestarconnections_connection" "main" {
  name          = "magi-app-stg-reports-github"
  provider_type = "GitHub"
  tags          = local.common_tags
}

resource "aws_codebuild_project" "main" {
  name          = "magi-app-stg-reports-build"
  build_timeout = 20
  ...
  logs_config {
    cloudwatch_logs {
      group_name  = "/codebuild/magi-app-stg-reports"
      stream_name = "build"
    }
  }
...
resource "aws_codepipeline" "main" {
  name     = "magi-app-stg-reports-pipeline"
