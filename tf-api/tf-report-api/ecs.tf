###############################################################################
# ecs.tf  –  ECS task definition, service, autoscaling for reports
###############################################################################

resource "aws_cloudwatch_log_group" "main" {
  name              = "/ecs/magi-app-stg-reports"
  retention_in_days = 14
  tags              = local.common_tags
}

resource "aws_ecs_task_definition" "main" {
  family                   = "magi-app-stg-reports"
  network_mode             = "awsvpc"
  ...
  container_definitions = jsonencode([{
    ...
    logConfiguration = {
      logDriver = "awslogs"
      options = {
        "awslogs-group"         = "/ecs/magi-app-stg-reports"
...
resource "aws_ecs_service" "main" {
  name            = "magi-app-stg-reports-service"
 ...
 #   container_name   = "magi-app-stg-reports"
 #   container_port   = 3000
 # }
...
resource "aws_appautoscaling_policy" "main" {
  name               = "magi-app-stg-reports-cpu-scaling"
